...
import { getPlaceByType } from "../config/DataApp";
...
export default function SinglePlaceType(props) {
...
useEffect(() => {
  ...
  useEffect(() => {
    getPlaceByType(id).then(response => {
        setData(response);
        setIsLoaded(true);
    })
  ...
  const loadMore = () => {
    ...
    getPlaceByType(id, page+1).then((response) => {
