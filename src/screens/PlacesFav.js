...
import { getFavPlaces } from "../config/DataApp";
import {map} from 'lodash';
import AppLoading from '../components/InnerLoading';
import NoContentFound from '../components/NoContentFound';
import { List, Avatar} from 'react-native-paper';

export default function PlacesFav(props) {
  ...
  const onClickItem = (id, title) => {
    props.navigation.navigate('placedetails', {id, title});
  };

  useEffect(() => {
    getFavPlaces().then((response) => {
        setItems(response);
        setIsLoaded(true);
    });
  }, [items]);
