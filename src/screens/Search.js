...
import { searchItems } from "../config/DataApp";
...
export default function Search(props) {
  ...
  const onChangeScreen = (type, id, title) => {
  ...
  const loadMore = () => {
    ...
    searchItems(searchQuery, page+1).then((response) => {
  ...
  useEffect(() => {
    ...
    if(size(searchQuery) >= 3){
      ...
      searchItems(searchQuery).then((response) => {
        setItems(response);
        setIsLoaded(true);
    })
...
<SafeAreaView>
    ...
    <TouchableOpacity key={i} activeOpacity={0.9} onPress={() => onChangeScreen(item.type, item.id, item.title)}>
