...
import { getFavOffers } from "../config/DataApp";
import {map} from 'lodash';
import AppLoading from '../components/InnerLoading';
import NoContentFound from '../components/NoContentFound';
import { List, Avatar} from 'react-native-paper';

export default function OffersFav(props) {
  ...
  const onClickItem = (id, title) => {
    props.navigation.navigate('offerdetails', {id, title});
  };

  useEffect(() => {
    getFavOffers().then((response) => {
        setItems(response);
        setIsLoaded(true);
    });
  }, [items]);
